...
文档级能力已通过真实 HTTP、PostgreSQL、MinIO、Milvus 和 Redis/Worker 链路核验：

- 同一知识库内按服务端重新计算的 SHA-256 识别完全重复；文件名不同也会命中，默认阻止重复入库。
- 同名但内容不同返回结构化冲突，由用户选择取消、保留两份或替换；不会自动覆盖。
- `keep_both` 在事务锁内分配不冲突名称，两份文件保持独立 `file_id` 和 `content_hash`。
- `replace` 先创建 inactive 候选，解析、清洗和索引成功后原子切换 active；旧版本在切换前继续可用。
- 同一目标的并发替换只有一个候选成功，其他请求返回 `replacement_in_progress`；失败候选不会永久占用目标。
- active 过滤对 vector、BM25 和 hybrid 查询一致生效；切换后 Worker 只清理旧 Milvus 在线向量，保留旧 chunks、Markdown、预览和原文件。

这些能力解决“文件是否重复、并存或安全替换”，不等同于知识级冲突检测。当前仅具备基于记录链的安全替换与历史保留；尚无面向用户的完整版本号、版本历史浏览和版本回滚页面，不应表述为完整版本管理系统。
