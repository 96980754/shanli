...
AI知识库 将上传文件先保存为原文件，再解析为 Markdown 并按知识库分块策略入库。OCR 引擎既可在系统设置中作为默认值配置，也可在知识库上传或临时附件解析时逐次选择；未显式选择时使用 `default_ocr_engine`。
...
解析器把引擎切出的图片转存到公开读的 `public` 桶，并把 Markdown 里的引用改写成可渲染的地址。各引擎的输出形态不同（相对路径 `imgs/xxx.jpg`、HTML `<img src>`、或只给临时地址），转存与改写由各解析器负责，但**出口统一兜底**：`parse_source_to_markdown` 会扫描结果里所有图片引用，凡是既不是 `http(s)` 也不是 `data:` 的，都会打出 `解析结果存在无法渲染的图片引用` 告警。正常情况下这条告警应为空；一旦出现，说明某个解析器漏了转存或漏了改写，需要修解析器而不是忽略。

### 图片输入会保留原图

对 `.jpg` / `.png` 等图片文件，OCR 引擎是把它当成一页文档来处理的：它只输出识别出的文字，以及它自己切出的图块（logo、印章这类碎片），**原图本身不会出现在结果里**。因此解析图片时系统会额外做两件事：

1. 把原图转存到 `public` 桶，作为一张整图放在解析结果最前面；
2. 丢弃引擎切出的碎片——输入本身就是一张图时，这些碎片按定义都是这张图的局部，原图已经完整保留，留着只会产生噪声。

结果就是"一张原图 + 识别出的文字"。需注意这会让原图进入匿名可读的对象存储路径。

### 表格属性会被剥离

引擎输出的表格带有 `border=1`、`style='margin: auto; word-wrap: break-word;'` 这类表现性属性（前端表格样式本就由 MarkdownPreview 统一控制，这些属性没有视觉作用），但会占掉大量字符——一份 42 万字符的认证报告 PDF 里有近 25 万字符是这类属性。解析出口会剥离表格标签上的 `style` 与 `border`，同时保留 `colspan`、`rowspan`、`align` 等有语义的属性，以及图片容器上的 `text-align`（那是图片居中的唯一手段）。
