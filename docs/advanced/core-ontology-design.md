...
admin 可以进入 Core Ontology 设置页查看全部版本的实体、关系、别名、属性和附加规则；只有 superadmin 可以创建、上传或编辑。内置 `general@1.0.0`“全领域通用”（新知识库图谱默认）与 `shanli-preset@4.3`“善理预设新版”始终只读；早期内置 `tongyong@1.0.0` 与 `shanli-preset@4.2` 已随迭代下架，切换 Core Ontology 前须先重置对应知识库图谱。
