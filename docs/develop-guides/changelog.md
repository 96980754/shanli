...
- 调用 skill 变慢的根因修复：实测技能激活首个 `read_file SKILL.md` 要阻塞约 4.1s，来自沙箱容器冷启动（docker 容器启动 + `/v1/sandbox` 健康探测），由空闲回收器造成重复冷启动——回收器原生效值为 210s（配置 120s 因 ≤ exec timeout 180s 被自动抬到 210s），对话间隔稍长（阅读/思考/再次提问）容器即被回收，下次提问重新冷启动。双管齐下：① 闲置回收时长 `SANDBOX_IDLE_TIMEOUT_SECONDS` 120→1800（30 分钟），`.env` 与 docker-compose 默认值同步调整，provisioner 健康检查确认生效；② 运行开始时后台预冷——`stream_agent_chat`/`stream_agent_resume` 在 run 启动即 `_schedule_sandbox_prewarm`（`provider.get(create_if_missing=True)`，作用域按 `meta.file_thread_id`/`skills_thread_id` 与子智能体独立），把 ~4s 冷启动藏到首轮 LLM 生成之后；热路径（缓存命中）亚毫秒，预冷失败只记 warning 不影响本次运行。补充 `test_chat_service_sandbox_prewarm.py` 3 个单测（作用域默认/子智能体独立/失败吞掉不传播）。
