...
- 修复知识库同一父目录可创建同名文件夹的问题，增加名称规范化、结构化冲突响应和并发唯一性保护。新建文件夹会写入 `normalized_name`；应用层仍会规范化比较该字段为空的历史记录，唯一索引仅约束已写入规范化名称的记录。正式环境需先扫描历史重复数据再回填，系统不会自动合并或删除既有文件夹。
...
- 补齐文档清洗待确认阶段的 QA 草稿闭环：关闭“自动确认清洗并入库”时，QA 可基于当前清洗草稿文本生成、编辑、新增和删除（草稿使用与入库相同的确定性分块器生成预览片段），绑定 cleaning_version 与正文哈希；确认清洗时一次性保存最终文本与 QA，索引完成后将 QA 证据自动重新绑定到正式 chunks，重复确认保持幂等；重新生成不会静默覆盖人工编辑，正文变更会把旧 QA 标记 possibly_outdated。开启“自动确认清洗并入库”时保持原有快速入库流程不变。
...
- 稳定 Router 权限集成测试的临时用户与部门清理顺序，避免软删除用户残留导致 teardown 外键错误。
