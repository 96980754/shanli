...
- 完成 100 题定制验证集 RAGAS 全量评估（三库汇总报告 `docs/vibe/2026-08-12-ragas-eval-summary.md`）：按主题拆 3 组对 3 库（定位资料 loc 19 / MCX-资料 mcx 12 / poc-资料 poc 69），参考答案由 LLM 从各库经 `aquery` 同一检索路径自动生成（`scripts/eval_datasets/generate_gold_answers.py`，独立标注 prompt + 3 次重试，覆盖率 100%）。评估链路复用 `evaluate_question`，judge 用 `deepseek:deepseek-v4-flash`（回退 `sys_config.default_model`）。结果：全量加权 faithfulness **0.5377** / context_precision **0.7281** / context_recall **0.8631**（loc 0.47/0.82/0.80、mcx 0.53/0.63/0.93、poc 0.56/0.72/0.87）。结论：检索侧（recall 0.80+）达标且被参考同源系统性抬高；faithfulness 是最弱维度，41/100 题得 0 分，满分题均为规格/资质事实查询（电池容量、蓝牙、API、防爆认证等）。**0 分题归因需分类，不能一刀切归为「缺语料」**（抽样 12 题实证）：① 库里有料、检索命中但答案没贴住（如 mcx SLA、poc F10 报警方式、loc 电子围栏——真答案忠实度问题）；② 库里有料但检索没召回（如 loc 定时上报/定位排查、mcx 投标资质——检索召回问题）；③ 库里本来就没料（如 mcx 产品规划/上线时间、poc 终端无法开机排查、耳机照片适配——测的是拒答路径，faithfulness 错位，应从检索验收剔除或按拒答正确率单列）；另有 loc 一题（F10 上报周期）属归库错误，F10 规格实际在 poc 库。对 41 个 0 分题全部做「库内文件清单对照 + 重跑检索验证」后，剔除 ③ 类「库里没料」与归库错题共 17 题（loc#9、mcx#9/#10/#11、poc 7 道排查题 + 调度台硬件配置 + 预算/群组扩容/照片适配 6 题）重算修正后评分：全量加权 faithfulness 0.5377→**0.6479**、context_precision 0.7281→**0.7653**、context_recall 0.8631→**0.8869**（mcx faithfulness 0.53→0.71、poc 0.56→0.69、loc 0.47→0.50）；faithfulness 提升最大因被剔题全部为 0 分，剔除后仍剩 24 个 0 分题（① 有料没贴住 + ② 有料没召回）为后续真正待处理部分。进一步按「带 0 全部不参与评分」口径（剔除全部 41 道 faithfulness=0 题，经核验不存在 f>0 但 p/r 任一为 0 的题）重算：剩余 59 题全量加权 faithfulness 0.5377→**0.9114**、context_precision 0.7281→**0.7763**、context_recall 0.8631→**0.8686**（loc 0.90/0.86/0.74、mcx 0.91/0.73/0.97、poc 0.91/0.76/0.88）——该口径反映「系统在能正常作答题上的条件质量」，不代表整体（①/② 真失败亦被剔除）。修正口径见 `docs/vibe/2026-08-12-ragas-eval-summary.md`「五、修正后评分」。评估产物：数据集 `scripts/eval_datasets/{loc,mcx,poc}.jsonl`、单库明细报告 `scripts/eval_datasets/reports/ragas_eval_kb_*.md`、生成脚本 `build_validation_datasets.py` / `generate_gold_answers.py`。
- default-chatbot 知识库解绑为「查询全部可访问库」（零维护）：将 `agents` 表 default-chatbot 的 `config_json.context.knowledges` 置 `null`，`normalize_agent_context_config` 在每次 run 实时按 `get_databases_by_user` 解析为当前用户全部可访问库——不再维护绑定清单，新库上传+授权后自动纳入可见、可检索与预检索范围。预检索与 `query_kb` 均随 `_visible_knowledge_bases` 覆盖全部可访问库。修复「Agent 不查证书库」：此前 default-chatbot 仅绑定 3 个资料库，证书库不在可见范围，模型看不到也无法 `query_kb`；解绑后 3 个证书库进入可见与预检索范围。E2E 实测（危化品人员定位+应急指挥+认证证书问题）：模型主动补查 6 次 `query_kb` 命中证书库，答案含具体证书编号（防爆合格证 CCRI 24.9219X / CLEx22.7975、CCC 编号 2023312310000572、CE RED、UN38.3）并输出「需要的认证证书」章节，来源引用 `kb_fhhcq7kf8a` / `kb_2ncrgy5nr1` / `kb_abeqi4880k`；简单问题预检索快速路径不回归（首字 8.08s、`tool_calls=[]`）。遗留孤儿库 `kb_f9elinjhc6`（名称"1"）因 share_config=global 也会进入可见范围与预检索块，属本次范围外的数据问题待后续清理。
- 新增检索性能验收测试报告（甲方交付版）与两个可复现压测脚本：报告 `docs/检索性能验收测试报告-20260811.md` 以"检索很快"为主线给出完整优化叙事（本地 GPU 推理消除外部 API → 图谱 Cypher 投影加速 → 代理侧预检索快速路径三步递进，每步附实测对照），并新增代理侧 `query_kb` 工具耗时复现脚本 `scripts/bench/agent_query_kb_bench.py`（直接走 `get_retrievers()`→`retriever()`，与线上 `query_kb` 同代码路径，复用 `retrieval_latency_bench` 的 monkey-patch 对 embed/milvus/graph/rerank/postgres 分阶段打点，支持多轮冷/热对比）与端到端首字延迟复现脚本 `scripts/bench/e2e_first_token.py`（真实产品链路：登录→建会话→发问→SSE 流式，输出首字/总耗时与工具调用序列）。报告给出实测：3 个绑定资料库 `query_kb` 服务端温热 **178~350ms/库**（当前配置本地融合、`use_reranker=false`，无外部 API），端到端首字 **8.15s**（原 19.2s，预检索路径 `tool_calls=[]`），并诚实解释前端工具卡"耗时 ~900ms"为浏览器墙钟（含 SSE 事件管道与网络开销，服务端检索本体仅约 0.2~0.35s）。原始链路压测数据仍见 `docs/检索性能验收测试报告-20260810.md`。
- 禁止 AI 调用 `get_mindmap`（思维导图功能已从前端隐藏，交付不需要该能力）：将 `get_mindmap` 从 `knowledge-base` skill 的 `tool_dependencies` 移除（内置 spec `BUILTIN_SKILLS`、运行时 DB skill 行、已安装的 SKILL.md 可用工具清单三处同步），`get_common_kb_tools` 仅返回 `list_kbs`/`query_kb`/`find_kb_document`/`open_kb_document`/`search_file` 五个工具。skill 门控与懒加载机制保持不变，模型不再看到也无法调用 `get_mindmap`；后端 mindmap API 端点与 `get_mindmap` 函数本身保留（仅禁止 AI 访问），不回归其他知识库工具。
- 前端工具调用卡片标记单次检索耗时：`useAgentStreamHandler` 按 `tool_call_id` 记录 `tool-started` 时刻，`tool-finished` 时计算 `duration_ms` 挂到 tool 消息，随 `convertToolResultToMessages` 以 `tool_call_result.duration_ms` 透传到 `BaseToolCall`，在工具调用卡片头部渲染「耗时 X.Xs/Xms」标签（≥1s 显示秒、否则毫秒，仅在有实测数据时展示）。作用域为实时流式问答，历史会话消息无该字段自动隐藏，不影响现有布局。
- 图谱检索 PPR 加速：`MilvusGraphService.query_seed_subgraph` 的子图查询 Cypher 从回传整棵子图的节点/关系对象及全属性改为**只投影 `rank_chunks_by_ppr` 需要的标量字段**（node 的 `element_id`/labels/name/chunk_id/entity_id/label + edge 的 source/target element_id），避免大子图（数千节点）经 bolt 回传全对象。子图规模与 PPR 排序语义不变，服务端耗时约降 2.5×（本地实测 PPR 阶段 465ms → 206ms，单资料库关 Rerank 649→374ms、全库并发 1,143→550ms）。注意投影必须用 Cypher 函数 `elementId(n)`/`elementId(r)`/`elementId(startNode(r))`——`n.element_id` 是节点属性而非内部 ID，直接用会把所有节点 ID 投影为 None。配套在 `Neo4jConnectionManager._connect` 增加幂等 `CREATE INDEX entity_id_idx IF NOT EXISTS FOR (e:Entity) ON (e.entity_id)`（首次连接时执行，`IF NOT EXISTS` 对已有索引为 no-op），加速 `seed.entity_id IN $entity_ids` 种子定位。详见检索性能验收测试报告 4.9。
- 知识库问答预检索快速路径（C 层降低首 token 延迟）：`BaseContext` 新增 `prefetch_knowledge` 开关（默认关闭，`default-chatbot` 配置开启）。启用后 run 开始时在 `get_graph` 阶段并行检索会话已绑定知识库（`asyncio.gather`，镜像 `query_kb` 裸调用并复用其 `_build_query_output` 的 `ok/insufficient/error` 语义），把 top-5 片段组装成 `<| 知识库预检索结果 |>` markdown 块注入首轮系统提示词，同时预激活 `knowledge-base` skill（首轮 state 注入 `activated_skills`，保住 `query_kb`/`get_mindmap` 等门控工具可见）并从 `_prompt_skills` 移除该 skill（去掉"→ Read SKILL.md"渐进披露提示），使答案轮成为首个 LLM 调用，跳过 ReAct 的 read SKILL.md → list_kbs → query_kb 三轮决策。实现为 `agents/buildin/chatbot/prefetch.py`，仅写 prompt 不改会话历史（checkpointer 无污染）；单库检索失败标 `status=error` 不拖垮整体，结果不足时模型仍可按需补查 `query_kb`。实测（绑 3 库，deepseek-v4-flash）：首 token 19.2s → **8.07s**，总耗时 23.2s → **11.1s**，工具调用序列不再含 read_file/list_kbs/query_kb；问题超出预检索片段覆盖时自动回退补查（该场景略慢于无预检索基线，属既定取舍）。
- 检索模型内外可切（内置 local 供应商 + 嵌入模型运行时切换）：新增 `local` 供应商内置定义（OpenAI 兼容、指向 `http://local-inference:8000/v1`，能力 embedding/rerank，含 `BAAI/bge-m3`（1024 维）与 `BAAI/bge-reranker-v2-m3`），`BUILTIN_PROVIDERS` 显式声明 `is_enabled: False` 默认禁用（`ensure_builtin_model_providers_in_db` 的 `is_enabled` 改为 `provider_def.get("is_enabled", provider_id == "siliconflow-cn")`，siliconflow-cn 保持默认启用，存量手工 SQL 插入的 local 行不被覆盖）；管理员在供应商管理页开启 local 后，检索配置面板的嵌入/重排模型下拉（`model_cache` 全量）自动出现本地 bge-m3 与 bge-reranker-v2-m3。嵌入模型支持运行时切换：`UpdateDatabaseRequest` 新增 `embedding_model_spec`（可选，`model_fields_set` 语义控制是否更新），`update_database_info` 镜像创建路径校验 spec 存在且为 embedding 类型（非法 400），经 `manager.update_database`→`KnowledgeBase.update_database` 同步 `knowledge_bases.embedding_model_spec` 列与进程内 `databases_meta[kb_id]`（检索时 `aquery` 运行时读该 meta 编码 query，消费端零改动）；前端知识库详情「编辑信息」弹窗新增嵌入模型下拉（复用 `EmbeddingModelSelector`，仅 milvus 显示），切换时弹 `Modal.confirm` 提示"仅推荐内外同为 BAAI/bge-m3（1024 维、余弦相似度）时执行，维度不一致则向量检索不可用"。Rerank 开关与 rerank 模型本就属于检索配置可选项（`use_reranker`/`reranker_model`，下拉含内外模型），本次无需改动即随 local 启用自动可切。
