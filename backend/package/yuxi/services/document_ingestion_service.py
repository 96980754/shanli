...
def _validate_basic_file_signature(filename: str, content: bytes) -> None:
    """轻量文件签名校验（不依赖 parser 大改）。

    - PDF：文件头必须为 %PDF-
    - 文本：不得包含 NUL 字节（二进制内容）
    其余格式仅做扩展名支持校验（is_supported_file_extension）。
    """
    ...
    async def create_uploaded_document(
        ...
        *,
        ...
        if not is_preprocessed:
            ...
            if not is_supported_file_extension(staged_filename):
                await self._delete_staged_object_if_unclaimed(kb_id, item, bucket_name, object_name)
                raise ValueError(f"Unsupported file type: {os.path.splitext(staged_filename)[1] or staged_filename}")
            # 轻量文件签名校验（不依赖 parser 大改）：PDF 文件头 + 文本文件 NUL 字节
            try:
                _validate_basic_file_signature(staged_filename, file_bytes)
            except ValueError:
                await self._delete_staged_object_if_unclaimed(kb_id, item, bucket_name, object_name)
                raise
