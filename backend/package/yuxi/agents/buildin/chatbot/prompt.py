...
KNOWLEDGE_REFUSAL_REPLY = "抱歉，在现有知识库中未找到相关依据，已通知管理员补充。"
...
# 保留该常量，兼容项目中可能存在的旧引用；当前不要求模型自行生成引用标签。
SOURCE_CITE_PROMPT = """
<| 引用来源 |>
...
def build_prompt_with_context(context):
    ...
    业务管理员配置的 ``context.system_prompt`` 放在基础规则之后、硬约束之前。
    这样既允许按客户业务补充术语、语气和输出模板，又避免业务配置覆盖身份保护、
    证据约束和统一拒答规则。
    """
