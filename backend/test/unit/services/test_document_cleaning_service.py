...
def _service(monkeypatch, record):
    ...
    service = DocumentCleaningService(file_repository=repository, cleaner=OptionalAIDocumentCleaner())
    saved = {}
...
async def test_manual_cleaning_waits_without_creating_chunks_or_indexing(monkeypatch):
    record = _record()
    service, _repository, _saved = _service(monkeypatch, record)
    index_calls = []
...
async def test_save_draft_uses_optimistic_version_and_does_not_index(monkeypatch):
    ...
    service, _repository, saved = _service(monkeypatch, record)
...
async def test_stale_editor_is_rejected_without_overwriting_current_draft(monkeypatch):
    ...
    service, repository, _saved = _service(monkeypatch, record)
...
async def test_cross_knowledge_base_file_id_is_not_readable(monkeypatch):
    service, _repository, _saved = _service(monkeypatch, _record(kb_id="kb-private"))
