      ...
      <div v-if="versionCandidates.length > 0 && props.canManage" class="conflict-files-panel">
        ...
        <div class="file-list-scroll">
          <div
            ...
            :key="item.uid"
            class="conflict-item version-candidate"
          >
            ...
            <div v-if="item.action === 'version'" class="version-target-row">
              ...
              >
                <a-select-option
                  v-for="file in item.sameNameFiles"
                  :key="file.file_id"
                  :value="file.file_id"
                >
                ...
                <a-select-option
                  v-if="
                    item.selectedFile &&
                    !item.sameNameFiles.some((file) => file.file_id === item.selectedFile.file_id)
                  "
                  :key="item.selectedFile.file_id"
                  :value="item.selectedFile.file_id"
                >
                  {{ item.selectedFile.filename }} ·
                  {{ formatFileTime(item.selectedFile.updated_at || item.selectedFile.created_at) }}
                </a-select-option>
              </a-select>
...
const applyDocumentSelection = (file) => {
  const index = versionCandidates.value.findIndex(
    (item) => item.uid === activeVersionCandidateUid.value
  )
  if (index < 0) return
...
const syncSameNameSelection = (candidate) => {
  candidate.selectedFile =
    candidate.sameNameFiles.find((file) => file.file_id === candidate.currentFileId) || null
}
...
const chunkData = async () => {
  ...
  for (const file of fileList.value) {
    ...
    if (
      ['.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.tif', '.gif', '.webp'].includes(ext) &&
      !isOcrEnabled.value
    ) {
      message.error({
