    ...
    <div v-for="(row, index) in rows" :key="index" class="env-row">
      ...
      <div class="env-value-field">
        ...
        <a-button
          ...
          :aria-label="$t(isValueHidden(row) ? 'agentCfg.viewEnvValue' : 'agentCfg.hideEnvValue')"
      ...
      <a-button size="small" type="text" danger @click="removeRow(index)">{{ $t('common.delete') }}</a-button>
    </div>
    <a-button @click="addRow" class="add-env">
      <template #icon><PlusOutlined /></template>
      {{ $t('agentCfg.addEnvVar') }}
    </a-button>
  </div>
</template>
